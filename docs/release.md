...
`/bump-version` コマンドでバージョンを更新し、PRを作成する。

### 2. CIでビルド成功を確認

PR作成時にCIワークフローが自動実行される。ビルドが成功することを確認し、mainブランチにマージする。
...
| ワークフロー | トリガー | 目的 |
|-------------|---------|------|
| CI | mainブランチへのPR | ビルド検証 |
| Release | `v*` タグのプッシュ | リリースビルド |

### CI

mainブランチへのPR作成・更新時に自動実行される。TypeScriptビルドとTauriアプリのビルドを検証する。
